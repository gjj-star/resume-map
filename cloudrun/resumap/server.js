...
async function runAnalyze(parsed) {
  ...
  try {
    if (MOCK) { logReq(role, 'mock', ''); return { code: 200, obj: mockResponse(role) }; }
    // 强制规则引擎（对比用，绕过内置 key，保证走纯关键词兜底）
    if (forceEngine === 'rule') {
      logReq(role, 'rule', '强制规则引擎(对比)');
      return { code: 200, obj: Object.assign(analyze(text, role), { engine: 'rule', note: '已按请求强制使用规则引擎（对比用，无 AI）' }) };
    }
    if (!apiKey) { logReq(role, 'rule', '无 API Key，已降级规则引擎'); return { code: 200, obj: Object.assign(analyze(text, role), { engine: 'rule', note: '无 API Key，已降级规则引擎' }) }; }
    const raw = await callLLM(apiKey, text, role);
    try { const out = mapLLM(raw, role); logReq(role, 'llm', ''); return { code: 200, obj: out }; }
    catch (mapErr) { logReq(role, 'rule', 'LLM 输出校验失败，已降级规则引擎'); return { code: 200, obj: Object.assign(analyze(text, role), { note: 'LLM 输出校验失败，已降级规则引擎' }) }; }
  } catch (e) {
    // 模型调用失败 → 规则引擎兜底
    logReq(role, 'rule', 'LLM 调用失败，已降级规则引擎：' + e.message);
    return { code: 200, obj: Object.assign(analyze(text, role), { note: 'LLM 调用失败，已降级规则引擎：' + e.message }) };
  }
}
